...
#first for mean
m_mean <- lmer(yi_mean ~ 1 + (1 |Article) , weights = 1/vi_mean, data=data_1, control=lmerControl(check.nobs.vs.nlev="ignore", check.nobs.vs.nRE="ignore"))
summary(m_mean)
...
con_sum = length(unique(schizo$ID[schizo$Diagnosis == "Schizophrenia"]))
s_sum = length(unique(schizo$ID[schizo$Diagnosis == "Control"]))
